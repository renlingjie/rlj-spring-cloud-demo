...
public class Controller {
    ...
    public String retry(@RequestParam(name = "timeout") int timeout){
        ...
        return "client2 retry success~";
    }

    @GetMapping("/error")
    public String error(){
        log.info("client2 is fail");
    ...
    public String timeout(@RequestParam(name = "timeout") int timeout){
        try {
            ...
            System.out.println("client2线程ID：" + Thread.currentThread().getId());
            Thread.sleep((long) timeout * 1000);
            System.out.println("client2 线程睡眠已经结束，继续执行了");
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        return "client2 has execute " + timeout + "s";
    }
}
